...
function init(){
    ...
    document.getElementById('flagged').style.backgroundColor='#ECEFF1'
    document.getElementById('flagged').style.color='#212121'
    document.getElementById('username').innerText = userdetails['name']
}
function displaySearchResults(result){

}
function displayflagged(response) {
        ...
        xhr.onreadystatechange= function () {
            if(xhr.readyState == 4){
                ...
                flag.onclick = (e)=>{
                    ...
                    xhr.open('POST','http://localhost:2000/deflag')
    ...
    xhr.onreadystatechange = function () {
        if (xhr.readyState == 4) {
            ...
            if(response.status == 1)
                displayflagged(response['flag'])
            else
                displayNothing()
        }
    }
    xhr.open('GET',`http://localhost:2000/getflagged?userid=${userdetails['userid']}`)
    xhr.send()
}
